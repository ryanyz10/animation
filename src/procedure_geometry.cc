...
void create_cylinder_mesh(LineMesh &cylinder_mesh)
{
	...
	constexpr int kCylinderGridSizeZ = 3;  // number of points in each direction
	float step_x = 1.0f / (kCylinderGridSizeX - 1);
	float step_z = 1.0f / (kCylinderGridSizeZ - 1);
	...
	//               cylinder
	for (int i = 0; i < kCylinderGridSizeZ; ++i)
	{
		p.x = -0.5f;
	...
	// line segments.
	for (int n = 0; n < kCylinderGridSizeX * kCylinderGridSizeZ; ++n)
	{
...
{
	axes_mesh.vertices.push_back(glm::vec4(0.0f, 0.0f, 0.5f, 1.0f));
	axes_mesh.vertices.push_back(glm::vec4(1.0f, 0.0f, 0.5f, 1.0f));
	axes_mesh.vertices.push_back(glm::vec4(0.0f, 1.0f, 0.5f, 1.0f));
